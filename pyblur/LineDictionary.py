...
class LineDictionary:
    ...
    def createNxNLines(self, n):
        ...
        Num = 2 * n - 2
        validLineAngles = np.linspace(0, 180, Num, endpoint=False)
        validLineAngles=[int(i) for i in validLineAngles]
        cnt = 0
        for i in range(int((n - 1) / 2), n):
            j = 0
            lines[validLineAngles[cnt]] = [i, j, n - 1 - i, n - 1 - j]
            cnt += 1
        ...
        for j in range(int((n + 1) / 2), n):
            i = n - 1
            lines[validLineAngles[cnt]] = [n - 1 - i, n - 1 - j, i, j]
            cnt += 1
